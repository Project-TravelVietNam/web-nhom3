...
const FavoriteController = {
    ...
    addFavorite: async (req, res) => {
        try {
            ...
            if (!validTypes.includes(type)) {
                return res.status(400).json({ message: "Loại không hợp lệ. Vui lòng chọn loại: 'cultural', 'history' hoặc 'local'." });
            }
            ...
            if (!item) {
                return res.status(404).json({ message: `Mục ${type} không tồn tại trong cơ sở dữ liệu.` });
            }
            ...
            if (!favorite) {
                ...
                if (favorite[`${type}s`].includes(itemId)) {
                    return res.status(400).json({ message: `Mục đã tồn tại trong danh sách yêu thích.` });
                }
    ...
    removeFavorite: async (req, res) => {
        try {
            ...
            if (itemIndex === -1) {
                return res.status(404).json({ message: "Mục không có trong danh sách yêu thích." });
            }
